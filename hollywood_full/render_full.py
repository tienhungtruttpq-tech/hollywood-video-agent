...
CACHE=Path('/home/user/.cache/hollywood_full');PREP=CACHE/'prepared';RENDER=CACHE/'render'
...
def motion(i,era,size,q):
    ...
    native=ACTORS[i][era]['downloaded_size'];amount=.012 if min(native)<450 else .024
...
def reveal(i,photo,q):
    ...
    if q>=1:return photo
    p=ease(q);a=np.asarray(photo);b=hold_image(i);mode=i%4
    if mode==0:
...
def preview():
    for page in range(3):
        ...
        canvas.save(ROOT/'work'/f'final_layout_{page+1}.jpg',quality=92)
    frame(.7).save(OUT/'Hollywood_8min_Poster.jpg',quality=95)
...
def encode_segment(index):
    ...
    cmd=[FF,'-hide_banner','-loglevel','warning','-y','-f','rawvideo','-vcodec','rawvideo','-pix_fmt','rgb24',
         '-s',f'{W}x{H}','-r',str(FPS),'-i','pipe:0','-an','-c:v','libx264','-preset','fast','-crf','20',
...
def metadata():
    lines=[';FFMETADATA1','title=Hollywood Then & Now — 30 Stars | Full 8-Minute Edition',
...
def render(workers=2):
    digest=hashlib.sha256(b''.join((ROOT/p).read_bytes() for p in ['render_full.py','actors.json','framing.json','timing.json'])).hexdigest()
    ...
    output=OUT/'Hollywood_Then_Now_8min.mp4';meta=metadata();mix=CACHE/'master_mix.wav'
    if not mix.exists():raise FileNotFoundError('Prepare the full music/voice mix with align_audio.py first.')
    command=[FF,'-hide_banner','-loglevel','warning','-y','-f','concat','-safe','0','-i',str(listing),
             ...
             '-c:v','libx264','-preset','medium','-crf','23','-threads','4','-pix_fmt','yuv420p','-c:a','aac','-b:a','192k','-ar','48000','-af','loudnorm=I=-16:TP=-1.5:LRA=9',
