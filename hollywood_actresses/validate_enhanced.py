...
ROOT=Path(__file__).resolve().parent;CACHE=Path('/home/user/.cache/hollywood_actresses_refresh')
FILE=ROOT/'output/Hollywood_Actresses_8min_Enhanced.mp4';FF=imageio_ffmpeg.get_ffmpeg_exe()
...
actors=json.loads((ROOT/'actors.json').read_text());timing=json.loads((ROOT/'timing_enhanced.json').read_text())['parts']
...
s=p.stderr[p.stderr.rfind('Summary:'):];(ROOT/'work/loudness_enhanced.txt').write_text(s)
...
srt=(ROOT/'output/Actresses_Enhanced_EN.srt').read_text()
def sec(x):
...
times=[.35,1.05,1.65,2.5,4.8,5.1,5.8,10.,10.7,25.8,63.8,117.4,163.,252.,289.,352.,451.,466.,473.,477.]
...
samples=[]
for t in [10.,10.7]:
 c.set(cv2.CAP_PROP_POS_MSEC,t*1000);ok,bgr=c.read();assert ok
 samples.append(bgr[610:712,0:40].astype(np.float32))
motion_delta=float(np.mean(np.abs(samples[1]-samples[0])))
assert motion_delta>.25,('Footer unexpectedly static',motion_delta)
c.release()
for name in ['Thumbnail_A_Enhanced.jpg','Thumbnail_B_Enhanced.jpg']:
 assert Image.open(ROOT/'output'/name).size==(1280,720)
result={'file':FILE.name,'width':w,'height':h,'fps':fps,'frames':frames,'duration_seconds':duration,'bytes':FILE.stat().st_size,'full_decode_exit':p.returncode,'actresses':24,'unique_photographs':48,'subtitle_cues':len(ranges),'first_name_reveal_seconds':5.0,'opening_montage_seconds':2.0,'first_guess_duration_seconds':3.0,'second_name_reveal_seconds':25.0,'audio_integrated_lufs':float(I.group(1)) if I else None,'audio_true_peak_dbfs':float(TP.group(1)) if TP else None,'audio_lra_lu':float(LRA.group(1)) if LRA else None,'font_headlines':'Outfit ExtraBold','font_captions':'Manrope SemiBold','footer_motion_mean_pixel_difference':motion_delta,'sha256':hashlib.sha256(FILE.read_bytes()).hexdigest()}
(ROOT/'work/validation_enhanced.json').write_text(json.dumps(result,indent=2));print(json.dumps(result,indent=2));print(s)
