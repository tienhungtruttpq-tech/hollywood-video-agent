...
CACHE=Path('/home/user/.cache/hollywood_actresses');PREP=CACHE/'prepared';WORK=CACHE/'render';WORK.mkdir(parents=True,exist_ok=True)
...
DATA=json.loads((ROOT/'timing.json').read_text());PARTS=DATA['parts'];BYID={p['id']:p for p in PARTS}
...
def font(size,face='sans',weight=500):
 ...
 if key not in FONTS:
  f=ImageFont.truetype(str(FONT_DIR/('BebasNeue-Regular.ttf' if face=='display' else 'Manrope.ttf')),int(size))
  if face!='display':
   try:f.set_variation_by_axes([weight])
   except Exception:pass
  FONTS[key]=f
 return FONTS[key]
def text(d,xy,s,size=20,fill=CREAM,face='sans',weight=500,anchor=None):d.text(xy,s,font=font(size,face,weight),fill=fill,anchor=anchor)
...
def layout(i,guess):
 key=(i,guess)
 if key not in BASES:
  a=ACTORS[i];p=APART[i];im=BACKGROUND.copy();d=ImageDraw.Draw(im)
  ...
  text(d,(640,55),title,52,CREAM,'display',anchor='mt')
  ...
  if not guess:
   text(d,(640,294),str(a['delta']),54,GOLD,'display',anchor='mt')
   ...
   track(d,(640,606),'THREE-SECOND CHALLENGE' if i==0 else 'ROLE CLUE',10,2.0,LAV,'center')
...
def caption(text_value):
 if text_value not in CAPTIONS:
  sz=23;lines=wrapped(text_value,1120,sz)
  while len(lines)>2 and sz>19:sz-=1;lines=wrapped(text_value,1120,sz)
  im=Image.new('RGBA',(W,64));d=ImageDraw.Draw(im)
  for j,line in enumerate(lines):text(d,(640,j*(sz+6)),line,sz,CREAM,anchor='mt')
...
def card(i,local,t,cold_open=False):
 ...
 im=layout(i,guess);im.paste(photo(i,'then'),(PX[0],PY))
 ...
 if guess:
  ...
  text(d,(640,322),str(remaining),61,CREAM,'display',anchor='mt')
  track(d,(640,403),'SECONDS',9,1.5,MUTED,'center')
  c=current_caption(BYID['intro'],t) if cold_open else None
  layer=caption(c or p['hint']);im.paste(layer,(0,635),layer)
 else:
  c=current_caption(p,t)
  if c:
   layer=caption(c);im.paste(layer,(0,635),layer)
   ...
   layer=caption(s);im.paste(layer,(0,635),layer)
...
def round_card(p,local):
 ...
 im=ROUND_PAGES[n].copy();d=ImageDraw.Draw(im)
 track(d,(640,213),f'{n*6} REVEALED / {24-n*6} TO GO',14,2.5,LAV,'center')
 text(d,(640,262),f'ROUND {n+1}',108,CREAM,'display',anchor='mt')
 ...
 if c:
  layer=caption(c);im.paste(layer,(0,641),layer)
 return im

def outro(t):
 im=photo_montage([0,1,4,18,22,23]);d=ImageDraw.Draw(im)
 track(d,(640,175),'TWENTY-FOUR FACES. DECADES OF MOVIE MEMORIES.',12,1.8,LAV,'center')
 text(d,(640,236),'YOUR SCORE / 24?',107,CREAM,'display',anchor='mt')
 ...
 if c:
  layer=caption(c);im.paste(layer,(0,632),layer)
 return im
...
def credits(page):
 ...
 if page==0:
  text(d,(58,95),'REAL PHOTOS. ORIGINAL QUIZ.',66,CREAM,'display')
  ...
  text(d,(58,80),'THE PHOTOGRAPHERS & ARCHIVES',51,CREAM,'display')
...
def frame(t):
 if t<3:im=card(0,t,t,True)
 elif t>=470:im=credits(0 if t<475 else 1)
...
def preview():
 ...
 for t in [.5,2.7,3.2,25.,116.,117.5,465.8,472.]:frame(t).save(WORK/f'frame_{t:.1f}.jpg',quality=93)
...
def encode_part(i):
 ...
 cmd=[FF,'-hide_banner','-loglevel','warning','-y','-f','rawvideo','-vcodec','rawvideo','-pix_fmt','rgb24','-s',f'{W}x{H}','-r',str(FPS),'-i','pipe:0','-an','-c:v','libx264','-preset','medium','-tune','stillimage','-crf','24','-threads','2','-g','96','-keyint_min','24','-pix_fmt','yuv420p','-movflags','+faststart',str(dest)]
...
def render(workers):
 checksum=hashlib.sha256(b''.join((ROOT/p).read_bytes() for p in ['render_episode.py','actors.json','framing.json','timing.json'])).hexdigest();stamp=WORK/'config.sha256'
 ...
 temp=WORK/'final_mux.mp4';target=OUT/'Hollywood_Actresses_Then_Now_Quiz_8min.mp4'
